...
function AddUser() {
    ...
    const firstNameRef = useRef(null)
      ...
      const handleAddUser = (event)=>{
           ...
           fetch("http://localhost:8000/dashboard/add-user",{
            method:"POST",
            headers:{
           ...
           .then((result)=>{ 
            alert(`User ${result.user.firstName + " " +result.user.lastName} added successfully`)
            
            if(location.state.source === 'AdminDashboard')
  ...
  <Card style={{ width: '30rem' }} className="p-5 shadow">
    ...
    <Form.Group as={Row} className="mb-5" controlId="formPlaintextFName">
      ...
      <div className="text-end d-flex justify-content-center">
        <button className="btn btn-primary  px-4 py-2" onClick={(e)=>handleAddUser(e)}>Add User</button>
      </div>
    </Form>
  </Card>
  </div>
